<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="消息发送时间" prop="sentAt">
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['messages:messages:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['messages:messages:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['messages:messages:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['messages:messages:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table v-loading="loading" :data="messagesList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="消息发送时间" align="center" prop="sentAt" width="180">
        <template #default="scope">
          <span>{{ parseTime(scope.row.sentAt) }}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width">
        <template #default="scope">
          <el-button link type="primary" icon="Edit" @click="handleUpdate(scope.row)" v-hasPermi="['messages:messages:edit']">修改</el-button>
          <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)" v-hasPermi="['messages:messages:remove']">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="messagesRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="所属会话ID" prop="conversationId">
...
import { listMessages, getMessages, delMessages, addMessages, updateMessages } from "@/api/messages/messages";
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm('是否确认删除消息编号为"' + _ids + '"的数据项？').then(function() {
    return delMessages(_ids);
  }).then(() => {
    getList();
...
function handleExport() {
  proxy.download('messages/messages/export', {
    ...queryParams.value
